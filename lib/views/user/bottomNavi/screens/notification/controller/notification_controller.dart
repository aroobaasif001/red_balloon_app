  ...
  void _bindNotifications() {
    ...
    _firestore
      ...
      final docs = snapshot.docs.map((doc) {
        ...
        data['id'] = doc.id;
        return data;
      }).where((n) => n['category'] != 'chat_message').toList(); // 🔥 Filter out chat messages

      allNotifications.assignAll(docs);
      
      // Filter Offers
      offerNotifications.assignAll(docs.where((n) => 
        n['category'] == 'offer_received' || 
        n['category'] == 'offer_accepted'
      ).toList());

      // Filter Validation Hub
      validationNotifications.assignAll(docs.where((n) => 
